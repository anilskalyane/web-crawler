...
class HREFParser(HTMLParser):
	...
	if tag == 'img':
	    dict_attrs1 = dict(attrs)
	    if dict_attrs1.get('src'):
		print dict_attrs1.get('src')
...
class CrawlerCache(object):
    ...
    def get_urls(self, domain):
        ...
        self.cursor.execute('SELECT url FROM sites WHERE domain=?',
                            (domain, ))
...
class Crawler(object):
        ...
        if max_depth:
            for url in urls:
                ...
                if url not in self.content:
                    html = self.get(url)
                    self.set(url, html)
                    n_urls = n_urls.union(get_local_links(html,
                            self.domain))
    ...
    def curl(self, url):
        ...
        try:
            ...
            req = urllib2.Request('%s://%s%s' % (self.scheme,
                                  self.domain, url))
